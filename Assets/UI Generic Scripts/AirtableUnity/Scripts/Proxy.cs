...
namespace AirtableUnity.PX
{
    public static class Proxy
    {
        ...
        )
        {
            ...
            do
            {
                yield return ListRecords(tableName, curOffset, filterByFormula).SendWebRequest(
                    (response) =>
                    {
                    ...
                    (response, request) =>
                    {
                        onFail?.Invoke(response, request);
                    });
        ...
        public static UnityWebRequest ListRecords(string tableName, string offset = "", string filterByFormula = "")
        {
            ...
            if (!string.IsNullOrEmpty(offset))
                relativeUri += $"&offset={offset}";

            if (!string.IsNullOrEmpty(filterByFormula))
                relativeUri += $"&filterByFormula={filterByFormula}";
